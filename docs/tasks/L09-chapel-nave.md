# L09 — Complete production review of Chapel Nave

**Status:** First-pass room implemented; production quality and browser/hardware acceptance remain open. **Owner:** game room production. **Dependencies:** SYS-01, SYS-02, SYS-03; preceding room Flooded Archive.

## Start with these inputs

1. [Location design](../../content/locations/chapel-nave/location-design.md).
2. [Exact puzzle and hint contract](../../content/locations/chapel-nave/investigations-and-puzzles.md).
3. [Actual scene writing](../../content/locations/chapel-nave/scenes-and-dialogue.md).
4. [Pressure and branch effects](../../content/locations/chapel-nave/pressure-and-outcomes.md).
5. [Asset inventory](../../content/locations/chapel-nave/assets.md).
6. [Architecture](../architecture/nexus.md) and [review protocol](../validation/protocol.md).

## Desired player result

Verify Elian’s identity and decide whether to release her. The player understands enough to choose among “Release Elian into the marked stair”, “Ask Elian to maintain the archive line” and reaches Parish House with the recorded state intact.

## Exact implementation anchors

Content: `src/kits/locations/chapel-nave/content/`. Rules: `src/kits/interaction/player-actions-kit/kit.js`. Output: `src/presentation/environment.js` and `projection.js`. Provider: `src/providers/three-scene.js`. Do not edit another room's facts to make this room pass.
...
2. Play the current room using ordinary visible controls. Required knowledge: vessel-truth, elian-location. Record where the goal or clue is unclear.
3. Verify the exact solution The long breathing. Try a plausible incorrect input; inspect the stated feedback and recover without reloading.
4. Review each local alternative, including release, line. Trace outgoing state into the next room; do not substitute branch count for cross-room proof.
