# L15 — Complete production review of Exit Road

**Status:** First-pass room implemented; production quality and browser/hardware acceptance remain open. **Owner:** game room production. **Dependencies:** SYS-01, SYS-02, SYS-03; preceding room Town Square.

## Start with these inputs

1. [Location design](../../content/locations/exit-road/location-design.md).
2. [Exact puzzle and hint contract](../../content/locations/exit-road/investigations-and-puzzles.md).
3. [Actual scene writing](../../content/locations/exit-road/scenes-and-dialogue.md).
4. [Pressure and branch effects](../../content/locations/exit-road/pressure-and-outcomes.md).
5. [Asset inventory](../../content/locations/exit-road/assets.md).
6. [Architecture](../architecture/nexus.md) and [review protocol](../validation/protocol.md).

## Desired player result

Acknowledge the consequence and decide what you carry home. The player understands enough to choose among “Leave Saint Orison”, “Stay until the record is safely copied” and reaches the selected ending with the recorded state intact.

## Exact implementation anchors

Content: `src/kits/locations/exit-road/content/`. Rules: `src/kits/interaction/player-actions-kit/kit.js`. Output: `src/presentation/environment.js` and `projection.js`. Provider: `src/providers/three-scene.js`. Do not edit another room's facts to make this room pass.
...
2. Play the current room using ordinary visible controls. Required knowledge: last-listen, last-look. Record where the goal or clue is unclear.
3. Verify the exact solution This is what I can tell you. Try a plausible incorrect input; inspect the stated feedback and recover without reloading.
4. Review each local alternative, including home, stay. Trace outgoing state into the next room; do not substitute branch count for cross-room proof.
