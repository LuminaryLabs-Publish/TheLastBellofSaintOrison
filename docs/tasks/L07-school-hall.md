# L07 — Complete production review of School Hall

**Status:** First-pass room implemented; production quality and browser/hardware acceptance remain open. **Owner:** game room production. **Dependencies:** SYS-01, SYS-02, SYS-03; preceding room Bell Tower Base.

## Start with these inputs

1. [Location design](../../content/locations/school-hall/location-design.md).
2. [Exact puzzle and hint contract](../../content/locations/school-hall/investigations-and-puzzles.md).
3. [Actual scene writing](../../content/locations/school-hall/scenes-and-dialogue.md).
4. [Pressure and branch effects](../../content/locations/school-hall/pressure-and-outcomes.md).
5. [Asset inventory](../../content/locations/school-hall/assets.md).
6. [Architecture](../architecture/nexus.md) and [review protocol](../validation/protocol.md).

## Desired player result

Cross-check four identities without erasing chosen aliases. The player understands enough to choose among “Copy the records with aliases intact”, “Ask June what should remain private” and reaches Flooded Archive with the recorded state intact.

## Exact implementation anchors

Content: `src/kits/locations/school-hall/content/`. Rules: `src/kits/interaction/player-actions-kit/kit.js`. Output: `src/presentation/environment.js` and `projection.js`. Provider: `src/providers/three-scene.js`. Do not edit another room's facts to make this room pass.
...
2. Play the current room using ordinary visible controls. Required knowledge: names-first-four, school-order. Record where the goal or clue is unclear.
3. Verify the exact solution June Arlen → Tomas Reed → Ada Wren → Niko Bell. Try a plausible incorrect input; inspect the stated feedback and recover without reloading.
4. Review each local alternative, including copy, ask. Trace outgoing state into the next room; do not substitute branch count for cross-room proof.
