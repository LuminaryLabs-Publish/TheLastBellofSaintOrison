# L14 — Complete production review of Town Square

**Status:** First-pass room implemented; production quality and browser/hardware acceptance remain open. **Owner:** game room production. **Dependencies:** SYS-01, SYS-02, SYS-03; preceding room Return Ascent.

## Start with these inputs

1. [Location design](../../content/locations/town-square/location-design.md).
2. [Exact puzzle and hint contract](../../content/locations/town-square/investigations-and-puzzles.md).
3. [Actual scene writing](../../content/locations/town-square/scenes-and-dialogue.md).
4. [Pressure and branch effects](../../content/locations/town-square/pressure-and-outcomes.md).
5. [Asset inventory](../../content/locations/town-square/assets.md).
6. [Architecture](../architecture/nexus.md) and [review protocol](../validation/protocol.md).

## Desired player result

Choose how to share the record without claiming what you cannot prove. The player understands enough to choose among “Leave copies with the residents”, “Arrange independent custody of the originals” and reaches Exit Road with the recorded state intact.

## Exact implementation anchors

Content: `src/kits/locations/town-square/content/`. Rules: `src/kits/interaction/player-actions-kit/kit.js`. Output: `src/presentation/environment.js` and `projection.js`. Provider: `src/providers/three-scene.js`. Do not edit another room's facts to make this room pass.
...
2. Play the current room using ordinary visible controls. Required knowledge: public-record, evidence-boundary. Record where the goal or clue is unclear.
3. Verify the exact solution Here is the proof, testimony and what remains unknown. Try a plausible incorrect input; inspect the stated feedback and recover without reloading.
4. Review each local alternative, including copies, originals. Trace outgoing state into the next room; do not substitute branch count for cross-room proof.
