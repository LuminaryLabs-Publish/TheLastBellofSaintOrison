# L03 — Complete production review of Closed Café

**Status:** First-pass room implemented; production quality and browser/hardware acceptance remain open. **Owner:** game room production. **Dependencies:** SYS-01, SYS-02, SYS-03; preceding room Visitor Centre.

## Start with these inputs

1. [Location design](../../content/locations/closed-cafe/location-design.md).
2. [Exact puzzle and hint contract](../../content/locations/closed-cafe/investigations-and-puzzles.md).
3. [Actual scene writing](../../content/locations/closed-cafe/scenes-and-dialogue.md).
4. [Pressure and branch effects](../../content/locations/closed-cafe/pressure-and-outcomes.md).
5. [Asset inventory](../../content/locations/closed-cafe/assets.md).
6. [Architecture](../architecture/nexus.md) and [review protocol](../validation/protocol.md).

## Desired player result

Compare the radio signal with Elian’s original cassette. The player understands enough to choose among “Keep the recording and leave”, “Trace the cable from the threshold” and reaches Market Street with the recorded state intact.

## Exact implementation anchors

Content: `src/kits/locations/closed-cafe/content/`. Rules: `src/kits/interaction/player-actions-kit/kit.js`. Output: `src/presentation/environment.js` and `projection.js`. Provider: `src/providers/three-scene.js`. Do not edit another room's facts to make this room pass.
...
2. Play the current room using ordinary visible controls. Required knowledge: true-signal, radio-channels. Record where the goal or clue is unclear.
3. Verify the exact solution 94 · tide station. Try a plausible incorrect input; inspect the stated feedback and recover without reloading.
4. Review each local alternative, including record, trace. Trace outgoing state into the next room; do not substitute branch count for cross-room proof.
