# L08 — Complete production review of Flooded Archive

**Status:** First-pass room implemented; production quality and browser/hardware acceptance remain open. **Owner:** game room production. **Dependencies:** SYS-01, SYS-02, SYS-03; preceding room School Hall.

## Start with these inputs

1. [Location design](../../content/locations/flooded-archive/location-design.md).
2. [Exact puzzle and hint contract](../../content/locations/flooded-archive/investigations-and-puzzles.md).
3. [Actual scene writing](../../content/locations/flooded-archive/scenes-and-dialogue.md).
4. [Pressure and branch effects](../../content/locations/flooded-archive/pressure-and-outcomes.md).
5. [Asset inventory](../../content/locations/flooded-archive/assets.md).
6. [Architecture](../architecture/nexus.md) and [review protocol](../validation/protocol.md).

## Desired player result

Recover the shutoff record and its carbon original. The player understands enough to choose among “Leave with the photographed record”, “Recover the tethered carbon original” and reaches Chapel Nave with the recorded state intact.

## Exact implementation anchors

Content: `src/kits/locations/flooded-archive/content/`. Rules: `src/kits/interaction/player-actions-kit/kit.js`. Output: `src/presentation/environment.js` and `projection.js`. Provider: `src/providers/three-scene.js`. Do not edit another room's facts to make this room pass.
...
2. Play the current room using ordinary visible controls. Required knowledge: archive-index, archive-safe. Record where the goal or clue is unclear.
3. Verify the exact solution Clinical → Oxygen → 02:13. Try a plausible incorrect input; inspect the stated feedback and recover without reloading.
4. Review each local alternative, including photo, original. Trace outgoing state into the next room; do not substitute branch count for cross-room proof.
