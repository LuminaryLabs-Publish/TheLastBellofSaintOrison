# L10 — Complete production review of Parish House

**Status:** First-pass room implemented; production quality and browser/hardware acceptance remain open. **Owner:** game room production. **Dependencies:** SYS-01, SYS-02, SYS-03; preceding room Chapel Nave.

## Start with these inputs

1. [Location design](../../content/locations/parish-house/location-design.md).
2. [Exact puzzle and hint contract](../../content/locations/parish-house/investigations-and-puzzles.md).
3. [Actual scene writing](../../content/locations/parish-house/scenes-and-dialogue.md).
4. [Pressure and branch effects](../../content/locations/parish-house/pressure-and-outcomes.md).
5. [Asset inventory](../../content/locations/parish-house/assets.md).
6. [Architecture](../architecture/nexus.md) and [review protocol](../validation/protocol.md).

## Desired player result

Recover the relay contract and the descent equipment. The player understands enough to choose among “Descend by the documented utility stair”, “Discuss the letter before descending” and reaches Maintenance Tunnels with the recorded state intact.

## Exact implementation anchors

Content: `src/kits/locations/parish-house/content/`. Rules: `src/kits/interaction/player-actions-kit/kit.js`. Output: `src/presentation/environment.js` and `projection.js`. Provider: `src/providers/three-scene.js`. Do not edit another room's facts to make this room pass.
...
2. Play the current room using ordinary visible controls. Required knowledge: safe-code, family-history. Record where the goal or clue is unclear.
3. Verify the exact solution 4 → 1 → 7. Try a plausible incorrect input; inspect the stated feedback and recover without reloading.
4. Review each local alternative, including utility, letter. Trace outgoing state into the next room; do not substitute branch count for cross-room proof.
