# L01 — Complete production review of Town Gate

**Status:** First-pass room implemented; production quality and browser/hardware acceptance remain open. **Owner:** game room production. **Dependencies:** SYS-01, SYS-02, SYS-03; working new-game flow.

## Start with these inputs

1. [Location design](../../content/locations/town-gate/location-design.md).
2. [Exact puzzle and hint contract](../../content/locations/town-gate/investigations-and-puzzles.md).
3. [Actual scene writing](../../content/locations/town-gate/scenes-and-dialogue.md).
4. [Pressure and branch effects](../../content/locations/town-gate/pressure-and-outcomes.md).
5. [Asset inventory](../../content/locations/town-gate/assets.md).
6. [Architecture](../architecture/nexus.md) and [review protocol](../validation/protocol.md).

## Desired player result

Read the crossing notice, then open a safe route into town. The player understands enough to choose among “Wait in shelter; cross on the dark lamp”, “Take the marked maintenance passage” and reaches Visitor Centre with the recorded state intact.

## Exact implementation anchors

Content: `src/kits/locations/town-gate/content/`. Rules: `src/kits/interaction/player-actions-kit/kit.js`. Output: `src/presentation/environment.js` and `projection.js`. Provider: `src/providers/three-scene.js`. Do not edit another room's facts to make this room pass.
...
2. Play the current room using ordinary visible controls. Required knowledge: crossing-rule, blank-record. Record where the goal or clue is unclear.
3. Verify the exact solution The blank record. Try a plausible incorrect input; inspect the stated feedback and recover without reloading.
4. Review each local alternative, including shelter, passage. Trace outgoing state into the next room; do not substitute branch count for cross-room proof.
