# Chapel Nave — implementation

Status: implementation and regression review
Owner: unassigned. Package: `src/kits/locations/chapel-nave`.

Outcome: Verify Elian’s identity and decide whether to release her. Improve public operations, dependency bindings and outgoing state while preserving reachable exit `parish-house`.
...
1. Enter from `flooded-archive` with the fixture equipment and knowledge.
2. Read obtainable clues: vessel-truth, elian-location.
3. Inspect the authored solution: The long breathing. Try a wrong input before the correct sequence.
4. Review alternatives release, line; confirm changed facts at the next location.
