# Parish House — implementation

Status: implementation and regression review
Owner: unassigned. Package: `src/kits/locations/parish-house`.

Outcome: Recover the relay contract and the descent equipment. Improve public operations, dependency bindings and outgoing state while preserving reachable exit `maintenance-tunnels`.
...
1. Enter from `chapel-nave` with the fixture equipment and knowledge.
2. Read obtainable clues: safe-code, family-history.
3. Inspect the authored solution: 4 → 1 → 7. Try a wrong input before the correct sequence.
4. Review alternatives utility, letter; confirm changed facts at the next location.
