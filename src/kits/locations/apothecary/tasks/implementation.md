# Apothecary — implementation

Status: implementation and regression review
Owner: unassigned. Package: `src/kits/locations/apothecary`.

Outcome: Test the resin, then prepare the lantern. Improve public operations, dependency bindings and outgoing state while preserving reachable exit `bell-tower-base`.
...
1. Enter from `market-street` with the fixture equipment and knowledge.
2. Read obtainable clues: resin-test, lantern-limits.
3. Inspect the authored solution: Triangular blue seal · dry. Try a wrong input before the correct sequence.
4. Review alternatives street, clinical; confirm changed facts at the next location.
