...
const forceCanvas = process.env.ORISON_FORCE_CANVAS === "1";
const out = forceCanvas ? "captures/browser/canvas" : "captures/browser";
...
try {
  ...
  assert.equal((await state()).graphics.mode, forceCanvas ? "canvas" : "webgl");
  async function click(id) {
    ...
    console.log("Clicked", id, "in", trace.at(-1).room);
  }
  ...
  await fs.writeFile(
    ...
    JSON.stringify(
      {
        mode: "real Chromium mouse input",
        trace,
        errors,
        final: await state(),
      },
  ...
  await fs.writeFile(
    out + "/failure.json",
    JSON.stringify({ message: error.message, errors, trace }, null, 2),
  );
  await page
    .screenshot({ path: out + "/failure.png", timeout: 3000 })
    .catch(() => {});
  throw error;
} finally {
  await context.close();
